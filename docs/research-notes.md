...
### 文字行列オフセットによる軽量 dewarp

透視補正後の画像を縦方向の複数 strip に分け、各 strip の文字行投影を中央 strip と照合して上下オフセットを推定します。十分な文字行数、信頼度、変形量がある場合だけ、列ごとのオフセットを補間して軽い紙面カーブを補正します。複雑な段組みや不安定な推定では補正せず、通常の透視補正結果をそのまま使います。

実装箇所:

- `src/clearscan_cv/dewarp.py`: `estimate_textline_column_offsets`, `dewarp_by_textline_columns`
- `src/clearscan_cv/static/index.html`: `estimateTextlineColumnOffsetsCanvas`, `dewarpCanvasByTextlineColumns`
- Web 版のページ表示と解析レポートに `文字行カーブ補正` を追加
...
さらに、二値化後の `ink_density` と `boldness_risk` が高い場合だけ、連通成分で小さな点・小字部品を保護しながら 3x3 erosion による筆画細化候補を作ります。候補が黒画素を落としすぎる、または edge density を落としすぎる場合は採用せず、過度な太字化だけを抑えるようにしています。

実装箇所:

- `src/clearscan_cv/pipeline.py`: `deshadow_luminance`, `preserve_high_frequency_detail`, `estimate_gatos_background`, `sauvola_threshold`, `to_clean_binary`, `_reduce_bold_binary_strokes`
- `src/clearscan_cv/static/index.html`: tile background 推定、`useFrequencyDeshadow`, `sauvolaThresholdValue`, `paperNoiseGuard`, `strokeContrast`, `reduceOverboldBinaryStrokes`
- テスト: 重影ページ、影付き・紙テクスチャノイズ付きページ、低コントラスト退化ページ、抗エイリアス文字ページ、高密度粗字ページで、空白領域・文字保持・太字化抑制を確認
...
| 深度 dewarp | Ma et al., "DocUNet", CVPR 2018 | 将来の曲面補正候補。現版ではモデルを同梱せず、文字行投影に基づく軽量 dewarp に留める。 |
| 3D dewarp | Das et al., "DewarpNet", ICCV 2019 | 3D shape ベースの dewarp は将来の別モデル候補。端末内版では軽量な 2D 列オフセット補正で対応。 |
...
| textline geometry | Feng et al., "DocGeoNet", ECCV 2022 | textline を幾何制約として扱う考えを、文字行 deskew と列オフセット dewarp に反映。 |
