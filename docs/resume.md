...
- Added Python/OpenCV and Web/Canvas lightweight textline-projection dewarping that estimates per-column vertical offsets and remaps mildly curved document photos before OCR.
...
- Added guarded stroke-thinning post-processing in both the browser and Python pipelines for dense binary scans, reducing overbold text while preserving small characters, punctuation, and edge density.
...
- Improved grayscale deshadowing with foreground-aware background estimation so dense text does not bias the estimated paper illumination surface.
...
- Added template-guided illumination correction for fixed forms and invoices, using an ideal template image to stabilize paper lighting before OCR/export.
