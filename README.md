...
Android Emulator でローカル後端まで含めて確認する場合は、`PocketCV_API35` AVD を用意してから QA スクリプトを実行します。スクリプトは debug APK のビルド、FastAPI 後端の起動、APK インストール、`API確認`、サンプル画像選択、`/api/process` 実行、スクリーンショット保存までを adb で検証します。
