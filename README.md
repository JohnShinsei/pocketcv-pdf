...
Android Emulator で端末内 OpenCV とローカル後端の両方を確認する場合は、`PocketCV_API35` AVD を用意してから QA スクリプトを実行します。スクリプトは debug APK のビルド、FastAPI 後端の起動、APK インストール、`API確認`、サンプル画像選択、端末内 OpenCV 処理、`/api/process` 実行、スクリーンショット保存までを adb で検証します。
